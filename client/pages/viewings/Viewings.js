...
Template.Viewings.helpers({
  ...
  participant: () => {
    return Participants.findOne({ _id: FlowRouter.getParam("participantId") });
  },
  stimulus: () => {
    return Stimuli.findOne({ _id: FlowRouter.getParam("stimulusId") });
  },
  number: () => {
    return parseInt(FlowRouter.getParam("number"));
  },
  period: () => {
    return Template.instance().period.get();
  },
  timestep: () => {
    return Template.instance().timestep.get();
  },
  includeIncomplete: () => {
    return Template.instance().includeIncomplete.get();
  },
  pointTrailLength: () => {
    return Template.instance().pointTrailLength.get();
  },
  hullParams: () => {
    return Template.instance().hullParams.get();
  }
});
