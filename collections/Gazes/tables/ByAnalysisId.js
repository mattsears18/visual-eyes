  ...
  columns: [
    {
      ...
      render(data, type, row, meta) {
        if (data) {
          return `<a href="/studies/${row.studyId}/participants/${
            row.participantId
          }">${data}</a>`;
        }
      },
    },
    {
      ...
      render(data, type, row, meta) {
        if (data) {
          return `<a href="/studies/${row.studyId}/stimuli/${
            row.stimulusId
          }">${data}</a>`;
        }
      },
      ...
      render(data, type, row, meta) {
        if (data) {
          return `<a href="/studies/${row.studyId}/analyses/${row.analysisId}/${
            row.participantId
          }/${row.stimulusId}/${row.number}">${data}</a>`;
        }
      },
    },
    {
      ...
      render(data, type, row, meta) {
        if (data) {
          ...
          data.fetch().forEach(function(aoi) {
            resp += `<div><a href="/studies/${row.studyId}/aois/${aoi._id}">${
              aoi.name
            }</div>`;
          });
          return resp;
