...
describe('PlotHull.getLastPointTrailTrace()', () => {
  it('gets the initial fixation trail trace', () => {
    ...
    chai.expect(trace.name).to.equal('Last 10 Gaze Points');
    chai.expect(trace.x).to.eql([ 100, 200, 300, 400, 500, 600 ]);
    chai.expect(trace.y).to.eql([ 400, 300, 200, 100, 700, 600 ]);
  });
  ...
  it('gets a last fixation trail trace (not initial)', () => {
    ...
    chai.expect(trace.x).to.eql([ 600, 700 ,800, 900, 100, 200, 300, 400, 500, 600 ]);
    chai.expect(trace.y).to.eql([ 600, 500, 400, 300, 200, 100, 400, 300, 200, 100 ]);
  });
});
