...
if(Meteor.isServer) {
  describe('PlotHull.getPointsTrace()', () => {
    it('gets fixations', () => {
      ...
      chai.expect(trace.name).to.equal('Fixations');
    });
    ...
    it('gets the initial points trace', () => {
      ...
      chai.expect(trace.name).to.equal('Gaze Points');
    ...
    it('gets a points trace (not initial)', () => {
      ...
      chai.expect(trace.x).to.eql([ 400, 500, 600, 700, 800, 900 ]);
      chai.expect(trace.y).to.eql([ 100, 700, 600, 500, 400, 300 ]);
    });
  });
}
