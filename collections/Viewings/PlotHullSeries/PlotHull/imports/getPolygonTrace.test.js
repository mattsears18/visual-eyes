...
describe('PlotHull.getPolygonTrace()', () => {
  it('gets the initial polygon trace', () => {
    ...
    chai.expect(trace.name).to.equal('Convex Hull');
    chai.expect(trace.x).to.eql([ 600, 500, 100, 400, 600 ]);
    chai.expect(trace.y).to.eql([ 600, 700, 400, 100, 600 ]);
  });
  ...
  it('gets a polygon trace (not initial)', () => {
    ...
    chai.expect(trace.x).to.eql([ 600, 300, 100, 200, 600 ]);
    chai.expect(trace.y).to.eql([ 100, 400, 200, 100, 100 ]);
  });
});
