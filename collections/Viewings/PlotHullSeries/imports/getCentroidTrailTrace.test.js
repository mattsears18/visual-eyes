...
if(Meteor.isServer) {
  describe('PlotHullSeries.getCentroidTrailTrace()', () => {
    it('gets the initial centroid trail trace', () => {
      ...
      chai.expect(trace.name).to.equal('Centroid Trail');
      chai.expect(trace.x).to.eql([ -10, -11 ]);
      chai.expect(trace.y).to.eql([ -10, -11 ]);
    });
    ...
    it('gets a centroid trail trace (not initial) for the first 5 hulls', () => {
      ...
      chai.expect(trace.x).to.eql([ 375, 450, 525, 600, 500 ]);
      chai.expect(trace.y).to.eql([ 416.6666666666667, 400, 383.3333333333333, 366.6666666666667, 400 ]);
    });
