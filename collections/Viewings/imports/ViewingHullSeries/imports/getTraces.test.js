...
describe('ViewingHullSeries.getTraces()', () => {
  ...
  it('gets the initial traces', () => {
    ...
    let traces = hullseries.getTraces({ initial: true, hullIndex: 0 })
    ...
    expect(traces.data[1].name).to.equal('Centroid Trail')
    expect(traces.data[1].x).to.eql([ -10, -11 ])
    expect(traces.data[1].y).to.eql([ -10, -11 ])

    expect(traces.data[2].name).to.equal('Last 6 Gaze Points')
    expect(traces.data[2].x).to.eql([ 100, 200, 300, 400, 500, 600 ])
    expect(traces.data[2].y).to.eql([ 400, 300, 200, 100, 700, 600 ])
  ...
  it('gets the traces (not initial)', () => {
    ...
    let traces = hullseries.getTraces({ hullIndex: 7})
    expect(traces.length).to.equal(6)

    expect(traces[0].x.length).to.equal(6)  // pointTrace
    expect(traces[0].y.length).to.equal(6)

    expect(traces[1].x.length).to.equal(8)  // centroidTrailTrace
    expect(traces[1].y.length).to.equal(8)

    expect(traces[2].x.length).to.equal(10) // pointTrailTrace
    expect(traces[2].y.length).to.equal(10)
