...
describe('Viewing.getSampledData()', () => {
  const testRows = [
  ...
  it('has an invalid step', () => {
    const viewing = Factory.create('viewing');
    expect(() => {
      viewing.getSampledData([], 1337);
    }).to.throw('invalidStep');

    expect(() => {
      viewing.getSampledData([], -2);
    }).to.throw('invalidStep');
  });
  ...
  it('has a step of zero', () => {
    const viewing = Factory.create('viewing');
    const rows = [];
    expect(viewing.getSampledData(rows, 0)).to.eql(rows);
  });
  ...
  it('samples the data at 20%', () => {
    ...
    expect(viewing.getSampledData(testRows, 20)).to.eql(expected);
  });
  ...
  it('samples the data at 10%', () => {
    ...
    expect(viewing.getSampledData(testRows, 10)).to.eql(expected);
  });

  it('can sample at any step value', () => {
    const viewing = Factory.create('viewing');
    expect(viewing.getSampledData(testRows, 0.5).length).to.equal(201);
  });
});
