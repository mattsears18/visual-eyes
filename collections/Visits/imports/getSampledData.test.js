...
describe('Visit.getSampledData()', () => {
  ...
  it('has an invalid step', () => {
    const visit = Factory.create('visit');
    expect(() => {
      visit.getSampledData([], 1337);
    }).to.throw('invalidStep');

    expect(() => {
      visit.getSampledData([], -2);
    }).to.throw('invalidStep');
  });
  ...
  it('has a step of zero', () => {
    const visit = Factory.create('visit');
    const rows = [];
    expect(visit.getSampledData(rows, 0)).to.eql(rows);
  });
  ...
  it('samples the data at 20%', () => {
    ...
    expect(visit.getSampledData(testRows, 20)).to.eql(expected);
  });
  ...
  it('samples the data at 10%', () => {
    ...
    expect(visit.getSampledData(testRows, 10)).to.eql(expected);
  });

  it('can sample at any step value', () => {
    const visit = Factory.create('visit');
    expect(visit.getSampledData(testRows, 0.5).length).to.equal(201);
  });
});
