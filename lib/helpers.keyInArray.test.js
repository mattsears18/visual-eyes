...
describe('lib/helpers.keyInArray()', () => {
  ...
  if(Meteor.isServer) {
    it('detects categories in a real SMI file', async () => {
      ...
      expect(helpers.keyInArray('category', await datafile.getRenamedRows())).to.be.true;
    });

    it('does not detect categories in a real imotions file', async () => {
      let datafile = Factory.create('imotionsDatafile');
      expect(helpers.keyInArray('category', await datafile.getRenamedRows())).to.be.false;
    });
  }
});
