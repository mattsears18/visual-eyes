...
Meteor.methods({
  'analyses.makeViewings'({ jobId, callback }) {
    ...
    let analysis = Analyses.findOne(job.data.analysisId);
    let stimulus = Stimuli.findOne(job.data.stimulusId);
    let participant = Participants.findOne(job.data.participantId);
