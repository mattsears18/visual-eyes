export default function makeViewingsByParticipantStimulus(analysis, stimulus, participant) {
  ...
  function checkMinTime() {
    ...
    if(potentialTotalTime > minViewingTime) {
      ...
      if(!(stimulus && stimulus.width && stimulus.width > 0 && stimulus.height && stimulus.height > 0)) {
        console.log('stimulus missing or invalid, stimulusId: ' + stimulus._id);
        status = 'invalidStimulus';
      } else {
        for(i = start; i <= end; i++) {
          // console.log('start: ' + start);
          // console.log('end: ' + end);
          ...
          gazepoints.push({
            ...
            x: +(gazepointsArr[i].x / stimulus.width).toFixed(10),
            y: +(gazepointsArr[i].y / stimulus.height).toFixed(10),
          });
        }
      }
