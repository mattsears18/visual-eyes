...
function preProcessDatafile(datafile, job, callback) {
  ...
  if(!datafile.rawRowCount || !datafile.rawRowsProcessed || (datafile.rawRowCount != datafile.rawRowsProcessed)) {
    csv({delimiter: "auto"})
...
function preprocessResults(results, datafile, job, callback) {
  let data = results;
  console.log('raw row count: ' + data.length);
  ...
  console.log('gazepoint count - numeric only: ' + data.length);

  data = getVisualIntakesOnly(data);
  console.log('gazepoint count - visual intakes only: ' + data.length);

  let dupGazepoints = getStimuliOnly(data);
  console.log('gazepoint count (with duplicates): ' + dupGazepoints.length);
  Datafiles.update({ _id: datafile._id }, { $set: { dupGazepointCount: dupGazepoints.length }});

  let gazepoints = getNonDuplicateCoordinatesInly(dupGazepoints);
  console.log('gazepoint count: ' + gazepoints.length);
  Datafiles.update({ _id: datafile._id }, { $set: { gazepointCount: gazepoints.length }});

  let fixations = getFixationsOnly(gazepoints);
  ...
  processGazepoints(data, datafile, job, callback);
}
...
function processGazepoints(cleanData, datafile, job, callback) {
  console.log('Process raw rows: ' + datafile._id);
