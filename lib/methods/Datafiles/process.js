...
function finishUp(datafileId, callback) {
  study = Studies.findOne({ datafileIds: datafileId });

  if(study && study.allDatafilesProcessed()) {
    Studies.update({ _id: study._id}, { $set: { datafilesProcessed: true, datafilesProcessing: false }});
  }
