...
Meteor.methods({
  'datafiles.process'({ jobDoc, callback }) {
    job = Jobs.getJob(jobDoc._doc._id);

    datafileId = job._doc.data.datafileId;
...
function preprocessResults(results, datafile, job, callback) {
  if(!datafile.processing) {
    if(!datafile.processed) {
      ...
      Datafiles.update({ _id: datafile._id }, { $set: { fileFormat: format }});
...
function processRecordings(cleanData, datafile, job, callback) {
  ...
  stimuliCount = Stimuli.find({ studyId: datafile.studyId }).count();

  console.log('stimuli type: ' + typeof(stimuli));
  console.log('stimuli is array: ' + Array.isArray(stimuli));
  console.log('stimuli length: ' + stimuli.length);
  console.log('stimuli count: ' + stimuliCount);
  ...
  study = Studies.findOne({ _id: datafile.studyId });

  if(study.datafiles().count() == study.datafilesProcessedCount()) {
