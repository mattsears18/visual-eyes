...
function preprocessResults(results, datafile, job, callback) {
  if(!datafile.processing) {
    if(!datafile.processed) {
      ...
      data = renameHeaders(data, datafile);
      data = removeNonNumericCoordinates(data);
...
function renameHeaders(data, datafile) {
  study = Studies.findOne(datafile.studyId);

  if(study) {
    if(study.datafileFormat == "smi") {
      console.log("SMI datafile format!");
    } else if(study.datafileFormat == "imotions") {
      console.log("iMotions datafile format!");
    } else {
      console.log()
    }
  }
...
function removeNonNumericCoordinates(data) {
  return data.filter(row => {
    return (Number.isInteger(parseInt(row.x)) && Number.isInteger(parseInt(row.y)));
  });
}

function fixationsOnly(data) {
