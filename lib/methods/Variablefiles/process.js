...
Meteor.methods({
  'variablefiles.process'({ variablefileId }) {
    check(variablefileId, String);

    variablefile = Variablefiles.findOne({ _id: variablefileId });

    if(variablefile) {
      study = Studies.findOne({ _id: variablefile.meta.studyId });

      if(study) {
        // Create any variables that don't already exist
        console.log('Create any variables that don\'t already exist');
        csv({delimiter: "auto"})
          .fromFile(variablefile.path)
